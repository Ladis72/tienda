...
void GestionPedidos::llenarTablaPedidos()
{
    ...
    for (int i = 0; i < consultaPedidos.numRowsAffected(); i++) {
        ...
        cabeceras << "ID" << "PROVEEDOR" << "PEDIDO" << "FECHA" << "LINEAS" << "ARICULOS" << "BASE" << "IVA" << "RE" << "TOTAL";
...
void GestionPedidos::on_pushButtonBorrar_clicked()
{
    if(pedidoSeleccionado.isEmpty()){
        QMessageBox::information(this,"ATENCIÓN","Primero debe seleccionar un pedido para poder borrarlo");
        return;
    }
    QMessageBox msgBox;
    msgBox.setText("Borrar documento");
    msgBox.setInformativeText("¿Quiere borrar el documento?.\nEsta operación no se puede deshacer ");
...
void GestionPedidos::on_pushButtonAceptar_clicked()
{
    ...
    AceptarPedido *aPedido = new AceptarPedido(pedidoSeleccionado,proveedor,docSeleccionado,fecha,this);
    aPedido->exec();
    if (aPedido->correcto) {
