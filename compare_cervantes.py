...
remote_schema = get_schema("cervantes19.ddns.net", "ladis", "meganizado72", "tiendaNueva")

with open("comparativa_cervantes.md", "w") as f:
    f.write("# Comparativa de Estructura de BD: `tiendaNueva` Local vs `tiendaNueva` (Cervantes)\n\n")
    
    if not local_schema:
        f.write("Error al conectar con la base de datos local.\n")
        exit()
    if not remote_schema:
        f.write("Error al conectar con la base de datos remota (Cervantes).\n")
        exit()
    ...
    if only_remote:
        f.write("## Tablas solo en Cervantes (Remoto)\n")
    ...
    for t in sorted(common):
        ...
        if only_remote_cols:
            table_diff.append(f"Columnas solo en Cervantes: " + ", ".join(f"`{c}`" for c in sorted(only_remote_cols)))
        ...
        for c in sorted(common_cols):
            ...
            if type_local != type_remote:
                table_diff.append(f"Columna `{c}` difiere en tipo: Local -> `{type_local}`, Cervantes -> `{type_remote}`")
...
print("Comparativa generada en comparativa_cervantes.md")
