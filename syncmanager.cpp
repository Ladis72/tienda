...
const QStringList SyncManager::TABLAS_MAESTRAS = {
    "familias",  "fabricantes", "proveedores",    "fpago",
    "impuestos", "formatos",    "motivosEntrada",
...
const QMap<QString, QStringList> SyncManager::CAMPOS_EXCLUIDOS = {
    {"articulos",
     {"stock", "min", "max", "pendientes_pedido", "ultima_venta",
      "ultimo_pedido", "encargados", "minimo_pedido"}},
    {"permisos", {"id"}},
    {"vales", {"idvales"}}};
...
SyncManager::SyncManager(QObject *parent)
    : QObject(parent), m_timerPing(new QTimer(this)),
      m_timerSync(new QTimer(this)), m_hayConexion(false), m_idTiendaLocal(0) {
  s_instance = this;

  // Comprobación de conexión cada 30 segundos
  m_timerPing->setInterval(2 * 60 * 1000);
...
void SyncManager::crearTablasSyncLocal() {
  ...
  // Índice para la purga periódica de registros ya subidos (subido=1, fecha
  // antigua). Comprobar por nombre de índice exacto para evitar duplicados.
  q.exec(
  ...
  if (q.next() && q.value(0).toInt() == 0) {
    q.exec(
        "ALTER TABLE sync_cola ADD INDEX idx_sync_cola_purga (subido, fecha)");
  }

  // Watermark para controlar cuándo se hizo la última purga (máx. una vez al
  // día)
  q.prepare(
      "INSERT IGNORE INTO sync_control (tabla) VALUES ('sync_cola_purga')");
  q.exec();
  ...
  for (const QString &tabla : TABLAS_MAESTRAS) {
    q.exec(QString("ALTER TABLE `%1` ADD COLUMN IF NOT EXISTS id_tienda_origen "
                   "INT DEFAULT NULL")
               .arg(tabla));
    q.exec(QString("ALTER TABLE `%1` ADD COLUMN IF NOT EXISTS updated_at "
    ...
    QString checkIdx =
        QString(
            ...
            "AND index_name = 'idx_%1_updated'")
            .arg(tabla);
    if (q.exec(checkIdx) && q.next() && q.value(0).toInt() == 0) {
      q.exec(QString("ALTER TABLE `%1` ADD INDEX idx_%1_updated (updated_at)")
                 .arg(tabla));
    }
  ...
  q.exec("UPDATE vales SET vale_uuid = UUID() WHERE vale_uuid = '' OR "
         "vale_uuid IS NULL");
  ...
  q.prepare(
      "INSERT IGNORE INTO sync_control (tabla) VALUES ('sync_unificaciones')");
  q.exec();
  ...
  // Índice único para que la subida de unificaciones sea idempotente (ON
  // DUPLICATE KEY)
  q.exec("ALTER TABLE sync_unificaciones ADD UNIQUE INDEX IF NOT EXISTS "
         "uq_unif (tabla, id_perdedor)");

  // Tabla de tombstones: registra qué filas se han borrado en la nube para
  // poder propagar el borrado a las demás tiendas (el download no ve filas
  // borradas).
...
void SyncManager::crearTriggers() {
  ...
  // Limpieza: eliminar triggers de tablas que ya no se sincronizan (como
  // 'directorios')
...
void SyncManager::crearTrigger(const QString &nombreTabla,
  ...
  QSqlDatabase db = QSqlDatabase::database(conf->getConexionLocal());
  QSqlQuery q(db);
  QString nombreTrigger =
      QString("sync_%1_%2").arg(nombreTabla).arg(evento.toLower());
  q.exec(QString("DROP TRIGGER IF EXISTS %1").arg(nombreTrigger));

  QString prefijo = (evento == "DELETE") ? "OLD" : "NEW";
  QString sql =
      QString("CREATE TRIGGER %1 AFTER %2 ON %3 FOR EACH ROW BEGIN "
              "  IF @skip_sync IS NULL OR @skip_sync = 0 THEN "
              "    INSERT INTO sync_cola (tabla, id_registro, accion) VALUES "
              "('%3', %4.%5, '%2'); "
              "  END IF; "
              "END")
          .arg(nombreTrigger, evento, nombreTabla, prefijo, clavePrimaria);
  if (!q.exec(sql)) {
    qDebug() << "SyncManager: Error creando trigger" << nombreTrigger << ":"
             << q.lastError().text();
  }
}
...
bool SyncManager::conectarNube() {
  ...
  QSqlQuery q(QSqlDatabase::database(conf->getConexionLocal()));
  q.exec("SELECT servidor, puerto, baseDatos, usuario, clave, ssl_ca FROM "
         "config_nube WHERE id = 1");
  if (!q.first())
    return false;
...
void SyncManager::sincronizar() {
  ...
  if (!m_hayConexion && !conectarNube()) {
    m_syncActivo = false;
    qDebug()
        << "SyncManager: Sincronización cancelada (sin conexión a la nube)";
    return;
  }

  qDebug() << "SyncManager: Iniciando ciclo de sincronización..."
  ...
  const QString usuario = conf->getUsuario(); // capturado en el hilo GUI
                                              // (seguro para el hilo de pool)
  ...
  m_futuroSync = QtConcurrent::run([=]() {
    ...
    QString connLocalClone =
        "SyncLocal_" + QUuid::createUuid().toString(QUuid::WithoutBraces);
    QString connNubeClone =
        "SyncNube_" + QUuid::createUuid().toString(QUuid::WithoutBraces);

    int subidos = 0, subUnif = 0, bajados = 0;
    bool okNube = false;
    {
      QSqlDatabase dbLocal =
          QSqlDatabase::addDatabase(driverLocal, connLocalClone);
      dbLocal.setHostName(hostLocal);
      if (portLocal > 0)
        dbLocal.setPort(portLocal);
      ...
      if (okLocal && okNube) {
        ...
        subUnif =
            self->subirUnificaciones(connLocalClone, connNubeClone, usuario);
        ...
        qDebug() << "SyncManager: Ciclo completado — Registros subidos:"
                 << subidos << "| Unificaciones subidas:" << subUnif
                 << "| Registros bajados:" << bajados;
      } else {
        qWarning() << "SyncManager: No se pudieron abrir las conexiones del "
                      "ciclo de sync"
                   << "(local:" << okLocal << ", nube:" << okNube << ")";
      }

      // Purga periódica de sync_cola (máx. una vez al día) usando la conexión
      // local clonada
      if (okLocal)
        self->purgarCola(connLocalClone, usuario);
    ...
    if (QSqlDatabase::contains(connNubeClone))
      QSqlDatabase::removeDatabase(connNubeClone);
    if (QSqlDatabase::contains(connLocalClone))
      QSqlDatabase::removeDatabase(connLocalClone);

    // Volver al hilo GUI para actualizar estado y emitir la señal
    QMetaObject::invokeMethod(
        self,
        [self, subidos, bajados, okNube]() {
          if (!self)
            return;
...
int SyncManager::subirCambios(const QString &connLocal, const QString &connNube,
                              const QString &usuario) {
  cargarIdTiendaLocal(connLocal);
  ...
  cola.exec("SELECT id, tabla, id_registro, accion FROM sync_cola WHERE subido "
            "= 0 ORDER BY fecha ASC");
  ...
  while (cola.next()) {
    ...
    if (TABLAS_TRANSACCIONALES.contains(tabla)) {
      if (accion == "DELETE") {
        ...
        if (tabla == "tickets") {
          del.prepare("DELETE FROM `tickets_nube` WHERE `id_tienda` = ? AND "
                      "`ticket` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        } else if (tabla == "lineasticket") {
          del.prepare("DELETE FROM `lineasticket_nube` WHERE `id_tienda` = ? "
                      "AND `id_local` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        } else if (tabla == "arqueos") {
          del.prepare("DELETE FROM `arqueos_nube` WHERE `id_tienda` = ? AND "
                      "`id_local` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        } else if (tabla == "pedidos") {
          del.prepare("DELETE FROM `pedidos_nube` WHERE `id_tienda` = ? AND "
                      "`id_local` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        } else if (tabla == "lineaspedido") {
          del.prepare("DELETE FROM `lineaspedido_nube` WHERE `id_tienda` = ? "
                      "AND `id_local` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        } else if (tabla == "salidaGenero") {
          del.prepare("DELETE FROM `salidas_nube` WHERE `id_tienda_origen` = ? "
                      "AND `id_local` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        } else if (tabla == "entradasSalidas") {
          del.prepare("DELETE FROM `entradasalida_nube` WHERE `id_tienda` = ? "
                      "AND `id_local` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        } else if (tabla == "lotes") {
          del.prepare("DELETE FROM `stock_tiendas_nube` WHERE `id_tienda` = ? "
                      "AND `id_local` = ?");
          del.addBindValue(m_idTiendaLocal);
          del.addBindValue(idReg);
        }
        ...
        if (reg.exec(sqlReg) && reg.first()) {
          ...
          if (tabla == "tickets") {
            ins.prepare(
                "INSERT INTO `tickets_nube` (id_tienda, ticket, usuario, "
                "cliente, fecha, hora, base, iva, descuento, total, fpago, "
                "cobrado, entrega, cambio) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                "ON DUPLICATE KEY UPDATE usuario=VALUES(usuario), "
                "cliente=VALUES(cliente), fecha=VALUES(fecha), "
                "hora=VALUES(hora), base=VALUES(base), iva=VALUES(iva), "
                "descuento=VALUES(descuento), total=VALUES(total), "
                "fpago=VALUES(fpago), cobrado=VALUES(cobrado), "
                "entrega=VALUES(entrega), cambio=VALUES(cambio)");
            ins.addBindValue(m_idTiendaLocal);
            ins.addBindValue(rec.value("ticket"));
            ins.addBindValue(
                rec.value("usuario").toInt() > 0 ? rec.value("usuario") : 1);
            ins.addBindValue(
                rec.value("cliente").toInt() > 0 ? rec.value("cliente") : 1);
            ...
            ins.prepare("INSERT INTO `lineasticket_nube` (id_tienda, id_local, "
                        "nticket, cod, descripcion, cantidad, iva, precio, "
                        "descuento, totallinea, fecha, hora) "
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                        "ON DUPLICATE KEY UPDATE nticket=VALUES(nticket), "
                        "cod=VALUES(cod), descripcion=VALUES(descripcion), "
                        "cantidad=VALUES(cantidad), iva=VALUES(iva), "
                        "precio=VALUES(precio), descuento=VALUES(descuento), "
                        "totallinea=VALUES(totallinea), fecha=VALUES(fecha), "
                        "hora=VALUES(hora)");
            ins.addBindValue(m_idTiendaLocal);
            ...
            ins.prepare(
                "INSERT INTO `arqueos_nube` (id_tienda, id_local, fecha, hora, "
                "ventasEfectivo, ventasTarjeta, entradas, efectivoReal, "
                "descuadre, efectivoContado, usuario) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                "ON DUPLICATE KEY UPDATE fecha=VALUES(fecha), "
                "hora=VALUES(hora), ventasEfectivo=VALUES(ventasEfectivo), "
                "ventasTarjeta=VALUES(ventasTarjeta), "
                "entradas=VALUES(entradas), efectivoReal=VALUES(efectivoReal), "
            ...
            ins.prepare(
                "INSERT INTO `pedidos_nube` (id_tienda, id_local, idProveedor, "
                "npedido, fechaPedido, nLineas, nArticulos, descuento, "
                "totalbase, totaliva, totalre, total, nFactura, notas) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                "ON DUPLICATE KEY UPDATE idProveedor=VALUES(idProveedor), "
                "npedido=VALUES(npedido), fechaPedido=VALUES(fechaPedido), "
                "nLineas=VALUES(nLineas), nArticulos=VALUES(nArticulos), "
                "descuento=VALUES(descuento), totalbase=VALUES(totalbase), "
                "totaliva=VALUES(totaliva), totalre=VALUES(totalre), "
                "total=VALUES(total), nFactura=VALUES(nFactura), "
                "notas=VALUES(notas)");
            ins.addBindValue(m_idTiendaLocal);
            ins.addBindValue(rec.value("id"));
            ...
            ins.prepare(
                "INSERT INTO `lineaspedido_nube` (id_tienda, id_local, "
                "nDocumento, idProveedor, cod, descripcion, cantidad, "
                "bonificacion, lote, fc, costo, descuento1, base, tipoIva, "
                "totalbase, iva, re, pvp) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                "ON DUPLICATE KEY UPDATE nDocumento=VALUES(nDocumento), "
                "idProveedor=VALUES(idProveedor), cod=VALUES(cod), "
                "descripcion=VALUES(descripcion), cantidad=VALUES(cantidad), "
                "bonificacion=VALUES(bonificacion), lote=VALUES(lote), "
                "fc=VALUES(fc), costo=VALUES(costo), "
                "descuento1=VALUES(descuento1), base=VALUES(base), "
                "tipoIva=VALUES(tipoIva), totalbase=VALUES(totalbase), "
                "iva=VALUES(iva), re=VALUES(re), pvp=VALUES(pvp)");
            ins.addBindValue(m_idTiendaLocal);
            ins.addBindValue(rec.value("id"));
            ...
            ins.prepare(
                "INSERT INTO `salidas_nube` (id_tienda_origen, id_local, cod, "
                "fechaEntrada, descripcion, cantidad, fechaCaducidad, pvp, "
                "idTienda_destino) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) "
                "ON DUPLICATE KEY UPDATE cod=VALUES(cod), "
                "fechaEntrada=VALUES(fechaEntrada), "
                "descripcion=VALUES(descripcion), cantidad=VALUES(cantidad), "
                "fechaCaducidad=VALUES(fechaCaducidad), pvp=VALUES(pvp), "
                "idTienda_destino=VALUES(idTienda_destino)");
            ins.addBindValue(m_idTiendaLocal);
            ins.addBindValue(rec.value("id"));
            ins.addBindValue(rec.value("cod"));
            ...
            ins.prepare(
                "INSERT INTO `entradasalida_nube` (id_tienda, id_local, fecha, "
                "hora, cantidad, idTiposRentrada, descripcion, usuario) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?) "
                "ON DUPLICATE KEY UPDATE fecha=VALUES(fecha), "
                "hora=VALUES(hora), cantidad=VALUES(cantidad), "
                "idTiposRentrada=VALUES(idTiposRentrada), "
                "descripcion=VALUES(descripcion), usuario=VALUES(usuario)");
            ins.addBindValue(m_idTiendaLocal);
            ...
            ins.prepare(
                "INSERT INTO `stock_tiendas_nube` (id_tienda, id_local, cod, "
                "lote, fecha, cantidad) "
                "VALUES (?, ?, ?, ?, ?, ?) "
                "ON DUPLICATE KEY UPDATE cod=VALUES(cod), lote=VALUES(lote), "
                "fecha=VALUES(fecha), cantidad=VALUES(cantidad)");
            ins.addBindValue(m_idTiendaLocal);
            ins.addBindValue(rec.value("id"));
            ...
            if (!fVal.isNull()) {
              if (fVal.userType() == QMetaType::QDate ||
                  fVal.userType() == QMetaType::QDateTime) {
                QDate d = fVal.toDate();
                if (d.isValid())
                  fStr = d.toString("yyyy-MM-dd");
              } else {
      ...
      QString pk = getPkTabla(tabla);
      QSqlQuery reg(dbLocal);
      // Usar consulta directa para evitar el bug del protocolo binario de
      // MySQL/MariaDB en Qt 6, el cual lee los campos de fecha como
      // QDateTime(Invalid) al usar prepare().
      QString sqlReg = QString("SELECT * FROM `%1` WHERE `%2` = '%3'")
                           .arg(tabla, pk, QString(idReg).replace("'", "''"));
      if (reg.exec(sqlReg) && reg.first()) {
        QSqlRecord rec = reg.record();
        ...
        for (int i = 0; i < rec.count(); ++i) {
          ...
          if (campo.toLower() == "id_tienda_origen" &&
              (rec.value(i).isNull() || rec.value(i).toInt() == 0)) {
          ...
          if (!val.isNull()) {
            if (val.userType() == QMetaType::QDateTime ||
                val.userType() == QMetaType::QDate) {
              QDateTime dt = val.toDateTime();
              // Si la fecha no es válida (p.ej. '0000-00-00'), la enviamos como
              // NULL para evitar errores de sintaxis/formato incorrecto en la
              // base de datos.
              if (dt.isValid()) {
          ...
          if (tabla == "tiendas") {
            bool esLocal = (rec.value("local").toInt() == 1 ||
                            rec.value("id").toInt() == m_idTiendaLocal);
            if (esLocal) {
              if (campo.toLower() == "local") {
                vStr =
                    "0"; // En la nube todas las tiendas deben tener local = 0
              } else {
                QStringList camposLocales = {"ip",        "usuario", "password",
                                             "baseDatos", "puerto",  "ssl_ca"};
        ...
        QString sql =
            QString(
                ...
                .arg(tabla, campos.join(","), valores.join(","),
                     updates.join(","));
        QSqlQuery ins(dbNube);
        ok = ins.exec(sql);
        if (!ok) {
  ...
  if (!procesados.isEmpty()) {
    QStringList sIds;
    for (qint64 id : procesados)
      sIds << QString::number(id);
    QSqlQuery(dbLocal).exec("UPDATE sync_cola SET subido = 1 WHERE id IN (" +
                            sIds.join(",") + ")");
  }
  return subidos;
}

int SyncManager::subirUnificaciones(const QString &connLocal,
                                    const QString &connNube,
                                    const QString &usuario) {
  QSqlDatabase dbLocal = QSqlDatabase::database(connLocal);
  QSqlDatabase dbNube = QSqlDatabase::database(connNube);
  QSqlQuery cola(dbLocal);
  cola.exec("SELECT id, tabla, id_perdedor, id_ganador FROM sync_unificaciones "
            "WHERE subido = 0 ORDER BY fecha ASC");
  ...
  while (cola.next()) {
    ...
    QSqlQuery ins(dbNube);
    ins.prepare(
        "INSERT INTO sync_unificaciones (tabla, id_perdedor, id_ganador) "
        "VALUES (?, ?, ?) "
        "ON DUPLICATE KEY UPDATE id_ganador = VALUES(id_ganador), fecha = "
        "CURRENT_TIMESTAMP");
  ...
  if (!procesados.isEmpty()) {
    ...
    for (qint64 id : procesados)
      sIds << QString::number(id);
    QSqlQuery(dbLocal).exec(
        "UPDATE sync_unificaciones SET subido = 1 WHERE id IN (" +
        sIds.join(",") + ")");
  }
  return subidos;
}

int SyncManager::bajarBorrados(const QString &connLocal,
                               const QString &connNube,
                               const QString &usuario) {
  QSqlDatabase dbLocal = QSqlDatabase::database(connLocal);
  QSqlDatabase dbNube = QSqlDatabase::database(connNube);
  ...
  QSqlQuery qN(dbNube);
  QString sql =
      QString("SELECT tabla, id_registro, UNIX_TIMESTAMP(fecha) AS f_ts "
  ...
  if (!borradosPendientes.isEmpty()) {
    ...
    qPurge.prepare("DELETE FROM sync_cola WHERE tabla = ? AND id_registro = ? "
                   "AND subido = 0");
  ...
  actualizarUltimaSync("sync_borrados",
                       QDateTime::fromSecsSinceEpoch(maxTs, Qt::UTC),
                       connLocal);
  if (borrados > 0)
    qDebug() << "SyncManager: Borrados aplicados localmente:" << borrados;
  if (conservados > 0)
    qDebug() << "SyncManager: Borrados conservados por edición local más nueva:"
             << conservados;
  return borrados;
}
...
void SyncManager::purgarCola(const QString &connLocal, const QString &usuario) {
  ...
  // Solo purgar una vez al día: el watermark evita recorrer/borrar la tabla en
  // cada ciclo (5 min).
  QDateTime ultimaPurga = ultimaSync("sync_cola_purga", connLocal, usuario);
  if (ultimaPurga.isValid() &&
      ultimaPurga.secsTo(QDateTime::currentDateTime()) < 24 * 3600)
    return;
  ...
  QSqlQuery q(dbLocal);
  int borradosCola = 0;
  if (q.exec("DELETE FROM sync_cola WHERE subido = 1 AND fecha < "
             "DATE_SUB(NOW(), INTERVAL 7 DAY)")) {
    borradosCola = q.numRowsAffected();
    if (borradosCola > 0)
      qDebug() << "SyncManager: Purga de sync_cola:" << borradosCola
  ...
  int borradosUnif = 0;
  if (q.exec("DELETE FROM sync_unificaciones WHERE subido = 1 AND fecha < "
             "DATE_SUB(NOW(), INTERVAL 7 DAY)")) {
    borradosUnif = q.numRowsAffected();
    if (borradosUnif > 0)
      qDebug() << "SyncManager: Purga de sync_unificaciones:" << borradosUnif
               << "registros eliminados";
  } else {
  ...
  actualizarUltimaSync("sync_cola_purga", QDateTime::currentDateTime(),
                       connLocal);
}

void SyncManager::registrarLog(const QString &connLocal,
                               const QString &categoria, const QString &usuario,
...
int SyncManager::bajarCambios(const QString &connLocal, const QString &connNube,
                              const QString &usuario) {
  cargarIdTiendaLocal(connLocal);
  QSqlDatabase dbLocal = QSqlDatabase::database(connLocal);
  QSqlDatabase dbNube = QSqlDatabase::database(connNube);
  ...
  // Usar consulta directa para evitar el bug del protocolo binario con
  // QDateTime(Invalid) en prepared statements.
  QString sqlU =
      QString("SELECT tabla, id_perdedor, id_ganador, fecha FROM "
              "sync_unificaciones WHERE fecha > '%1' ORDER BY fecha ASC")
  ...
  for (const QString &tabla : TABLAS_MAESTRAS) {
    ...
    qDebug() << "   -> Consultando tabla:" << tabla << "desde"
             << desde.toString("yyyy-MM-dd HH:mm:ss");

    // Usar consulta directa para evitar el bug del protocolo binario de MariaDB
    // que devuelve QDateTime(Invalid) UNIX_TIMESTAMP(updated_at) devuelve un
    // entero, inmune a ese bug, y permite comparar local vs nube de forma
    // independiente de la zona horaria.
    QString sqlN =
        QString("SELECT *, UNIX_TIMESTAMP(updated_at) AS up_ts FROM `%1` WHERE "
                "updated_at > '%2' ORDER BY updated_at ASC")
    ...
    qPendiente.prepare("SELECT COUNT(*) FROM sync_cola WHERE tabla = ? AND "
                       "id_registro = ? AND subido = 0");

    {
      ...
      while (qN.next()) {
        ...
        bool tieneLocal = (qTsLocal.exec() && qTsLocal.first() &&
                           !qTsLocal.value(0).isNull());
        if (tieneLocal) {
          qint64 localTs = qTsLocal.value(0).toLongLong();
          if (localTs >= recUpdTs) {
            qPendiente.bindValue(0, tabla);
            qPendiente.bindValue(1, idReg);
            bool pendiente = (qPendiente.exec() && qPendiente.first() &&
                              qPendiente.value(0).toInt() > 0);
            if (pendiente)
              continue; // la edición local pendiente ganará al re-subirse
          } else {
            ...
            qPendiente.bindValue(0, tabla);
            qPendiente.bindValue(1, idReg);
            if (qPendiente.exec() && qPendiente.first() &&
                qPendiente.value(0).toInt() > 0) {
              QSqlQuery qLimpiar(dbLocal);
              qLimpiar.prepare("DELETE FROM sync_cola WHERE tabla = ? AND "
                               "id_registro = ? AND subido = 0");
        ...
        for (int i = 0; i < rec.count(); ++i) {
          ...
          // up_ts es una columna sintética añadida por nosotros, no existe en
          // la tabla.
          if (campo == "up_ts")
            continue;
          // NUNCA sincronizar el ID físico local ni la PK aquí (se añade luego)
          if (campo == pk || campo.toLower() == "id" + tabla.toLower())
            continue;
          if (recLoc.indexOf(campo) == -1 ||
              excluidos.contains(campo.toLower()))
            continue;
          if (tabla == "tiendas") {
            // El campo 'local' es una configuración exclusiva de cada equipo/máquina y NUNCA debe sobrescribirse desde la nube
            if (campo.toLower() == "local") {
              continue;
            }
            bool esLocal = (idReg.toInt() == m_idTiendaLocal);
            if (esLocal) {
              QStringList camposLocales = {"ip",        "usuario", "password",
                                           "baseDatos", "puerto",  "ssl_ca"};
              if (camposLocales.contains(campo.toLower())) {
          ...
          if (!val.isNull()) {
            if (val.userType() == QMetaType::QDateTime ||
                val.userType() == QMetaType::QDate) {
              QDateTime dt = val.toDateTime();
              // Evitar que fechas no válidas (como '0000-00-00') generen un
              // string vacío y provoquen un error de "Incorrect date value: ''"
              // al insertar en la base de datos local.
              if (dt.isValid()) {
                vStr = "'" + dt.toString("yyyy-MM-dd HH:mm:ss") + "'";
              } else {
                vStr = "NULL";
    ...
    if (!avisosPrecios.isEmpty()) {
      qDebug() << "   [i] Creando nota de aviso para" << avisosPrecios.size()
               << "cambios de precio...";
      QSqlQuery qNot(dbLocal);
      // El esquema real usa enum('Alta','Normal','Baja') para prioridad
      // y enum('Pendiente','Completada') para estado.
      qNot.prepare("INSERT INTO notas (titulo, descripcion, usuario, "
                   "fecha_limite, prioridad, estado) VALUES (?,?,?,?,?,?)");
      ...
      if (!qNot.exec()) {
        QString err =
            "Error al crear nota de precios: " + qNot.lastError().text();
        qWarning() << "   [!]" << err;
        registrarLog(connLocal, "SyncError", usuario, err);
      } else {
        qDebug() << "   [+] Nota de aviso creada correctamente.";
        avisosPrecios.clear(); // Limpiar para que no se repita en la siguiente
                               // tabla si hubiera errores
      }
    }

    // 2.4: watermark por tabla, solo avanza si esta tabla bajó registros
    // nuevos. El contador es por tabla (no global) para evitar que una tabla
    // activa arrastre el watermark de las inactivas hacia atrás.
    if (bajadosTabla > 0) {
      // Salvaguarda anti-retroceso: el watermark nunca debe ser menor que el
      // actual.
      if (maxUpdate < ultimaSyncActual)
        maxUpdate = ultimaSyncActual;
      actualizarUltimaSync(tabla, maxUpdate, connLocal);
    }
    bajados += bajadosTabla;
  }
  qDebug() << "SyncManager: bajarCambios — bajados:" << bajados
           << "| borrados:" << borrados;
  return bajados;
}

void SyncManager::actualizarUltimaSync(const QString &tabla,
  ...
  // QDateTime(Invalid)
  QString sql =
      QString("UPDATE sync_control SET ultima_sync = '%1' WHERE tabla = '%2'")
          .arg(momento.toString("yyyy-MM-dd HH:mm:ss"), tabla);
  q.exec(sql);
}

void SyncManager::cargarIdTiendaLocal(const QString &connLocal) {
  QString conn = connLocal.isEmpty() ? conf->getConexionLocal() : connLocal;
...
QDateTime SyncManager::ultimaSync(const QString &tabla,
  ...
  QSqlQuery q(db);
  // Usar consulta directa para evitar el bug del protocolo binario de
  // MySQL/MariaDB en Qt 6, que retorna QDateTime(Invalid) en campos de
  // fecha/hora cuando se preparan consultas (prepare()).
  QString sql =
      QString("SELECT ultima_sync FROM sync_control WHERE tabla = '%1'")
          .arg(tabla);
  if (!q.exec(sql)) {
    QString err =
        QString("Error en ultimaSync para %1 (conexión %2, abierta: %3): %4")
            .arg(tabla, connName, db.isOpen() ? "sí" : "no",
                 q.lastError().text());
    qWarning() << "SyncManager:" << err;
    registrarLog(connLocal, "SyncError", usuario, err);
  } else if (q.first()) {
...
void SyncManager::sincronizarLotesInicial(const QString &connLocal,
                                          const QString &connNube,
                                          const QString &usuario) {
  Q_UNUSED(usuario);
  QSqlDatabase dbLocal = QSqlDatabase::database(connLocal);
  ...
  if (!dbLocal.isOpen() || !dbNube.isOpen())
    return;
  ...
  // Consultar tiendas reales en la base de datos para obtener el id exacto de
  // cada tienda
  ...
  if (qTiendas.exec("SELECT id, nombre FROM tiendas")) {
    while (qTiendas.next()) {
      ...
      if (idT == m_idTiendaLocal) {
        ...
        QSqlQuery qL(dbLocal);
        if (qL.exec("SELECT id, ean, lote, DATE_FORMAT(fecha, '%Y-%m-%d') as "
                    "fecha_fmt, cantidad FROM lotes WHERE cantidad <> 0")) {
          QStringList rows;
          int count = 0;
          while (qL.next()) {
            ...
            if (rows.size() >= 500) {
              QSqlQuery ins(dbNube);
              ins.exec(QString("INSERT INTO `stock_tiendas_nube` (`id_tienda`, "
                               "`id_local`, `cod`, `lote`, `fecha`, "
                               "`cantidad`) VALUES %1")
                           .arg(rows.join(",")));
              count += rows.size();
              rows.clear();
          ...
          if (!rows.isEmpty()) {
            ...
            ins.exec(
                QString(
                    "INSERT INTO `stock_tiendas_nube` (`id_tienda`, "
                    "`id_local`, `cod`, `lote`, `fecha`, `cantidad`) VALUES %1")
                    .arg(rows.join(",")));
            count += rows.size();
            rows.clear();
          }
          qDebug()
              << "SyncManager: Sincronizado stock en nube para tienda local"
              << nombreT << "(ID" << idT << ") - Lotes:" << count;
        }
      }
    }
  }
...
void SyncManager::prepararTablasRemotas() {
  ...
  for (const QString &tabla : TABLAS_MAESTRAS) {
    // 0. Asegurar que la tabla existe en la nube (copiando esquema local si
    // falta)
    qN.exec(QString("SHOW TABLES LIKE '%1'").arg(tabla));
    if (!qN.next()) {
      qDebug() << "SyncManager: La tabla" << tabla
               << "no existe en la nube. Intentando crearla...";
      if (qL.exec(QString("SHOW CREATE TABLE `%1`").arg(tabla)) && qL.next()) {
        ...
        if (qN.exec(createSql)) {
          qDebug() << "SyncManager: Tabla" << tabla
                   << "creada con éxito en la nube.";
        } else {
        ...
        qWarning()
            << "SyncManager: No se pudo obtener el esquema local para la tabla"
            << tabla;
        continue;
      }
    }

    // 1. Asegurar columnas de sincronización (por si acaso la tabla ya existía
    // pero era antigua)
    qN.exec(QString("ALTER TABLE `%1` ADD COLUMN IF NOT EXISTS "
                    "id_tienda_origen INT DEFAULT NULL")
                .arg(tabla));
    qN.exec(
        QString(
            "ALTER TABLE `%1` ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP "
    ...
    QString pk = getPkTabla(tabla);
    if (!pk.isEmpty() &&
        pk != "vale_uuid") { // vale_uuid se maneja aparte abajo
      qN.exec(
          QString(
              "SELECT COUNT(*) FROM information_schema.statistics "
              "WHERE table_schema = (SELECT DATABASE()) AND table_name = '%1' "
      ...
      if (qN.next() && qN.value(0).toInt() == 0) {
        qDebug() << "SyncManager: Añadiendo PK faltante a" << tabla
                 << "en la nube";
        if (!qN.exec(QString("ALTER TABLE `%1` ADD PRIMARY KEY (`%2`)")
                         .arg(tabla, pk))) {
    ...
    qN.exec(
        QString(
            ...
            .arg(tabla));
    if (qN.next() && qN.value(0).toInt() == 0) {
      qN.exec(QString("ALTER TABLE `%1` ADD INDEX idx_%1_updated (updated_at)")
                  .arg(tabla));
    }
  }
  ...
  qN.exec("ALTER TABLE `vales` ADD COLUMN IF NOT EXISTS "
          "vale_uuid CHAR(36) NOT NULL DEFAULT ''");
  qN.exec("UPDATE vales SET vale_uuid = UUID() WHERE vale_uuid = '' OR "
          "vale_uuid IS NULL");
  ...
  // SEC-02: Asegurar columna salt y ampliar el tamaño de la clave en la tabla
  // usuarios de la nube Esto evita que los hashes SHA-256 se trunquen o que
  // falle la inserción de salt en la nube.
  qN.exec("ALTER TABLE `usuarios` ADD COLUMN IF NOT EXISTS `salt` VARCHAR(64) "
          "DEFAULT NULL");
  ...
  // sync_borrados: tombstones que permiten propagar los borrados a otras
  // tiendas.
  ...
  for (const QString &tabla : TABLAS_MAESTRAS) {
    ...
    QString sql = QString("CREATE TRIGGER %1 AFTER DELETE ON `%2` FOR EACH ROW "
                          "INSERT INTO sync_borrados (tabla, id_registro) "
                          "VALUES ('%2', OLD.`%3`) "
                          "ON DUPLICATE KEY UPDATE fecha = CURRENT_TIMESTAMP")
                      .arg(nombreTrigger, tabla, pk);
    if (!qN.exec(sql)) {
      QString err =
          QString("Error creando trigger de borrados para %1 en la nube: %2")
              .arg(tabla, qN.lastError().text());
      qWarning() << "SyncManager:" << err;
      registrarLog(connLocal, "SyncError", conf->getUsuario(), err);
    }
...
void SyncManager::resincronizarTodoElStockNube() {
  ...
  // 2. Limpiar registros espurios en stock_tiendas_nube (ej. id_tienda = 0 o
  // tiendas eliminadas)
  if (!idsValidos.isEmpty()) {
    ...
    qClean.exec(QString("DELETE FROM stock_tiendas_nube WHERE id_tienda NOT IN "
                        "(%1) OR id_tienda = 0")
                    .arg(idStrs.join(",")));
  } else {
    QSqlQuery qClean(dbNube);
  ...
  if (qLoc.exec("SELECT id FROM tiendas WHERE local = 1 LIMIT 1") &&
      qLoc.first()) {
  ...
  for (auto it = tiendas.begin(); it != tiendas.end(); ++it) {
    ...
    if (idT == idTiendaLocal) {
      dbTienda = dbLocal;
    } else if (QSqlDatabase::contains(nombreT) &&
               QSqlDatabase::database(nombreT).isOpen()) {
    ...
    if (dbTienda.isOpen()) {
      ...
      if (qL.exec("SELECT id, ean, lote, DATE_FORMAT(fecha, '%Y-%m-%d') as "
        ...
        qDebug() << "SyncManager: Resincronizado stock en nube para tienda"
                 << nombreT << "(ID" << idT << ") ->" << count << "lotes";
      }
    }
  }
}
