...
void VerFacturas::llenarTabla()
{
    ...
    if (ui->checkBoxTodosProveedores->isChecked()) {
        if (tipoDocumento == "facturas") {
            sentenciaSql = "SELECT * FROM facturas WHERE fechaFactura >= '" + fechaInicial
                           + "' AND fechaFactura <= '" + fechaFinal + "'";
        } else {
            sentenciaSql = "SELECT * FROM albaranes WHERE fechaFactura >= '" + fechaInicial
                           + "' AND fechaFactura <= '" + fechaFinal + "'";
        }
    } else {
        if (tipoDocumento == "facturas") {
            ...
            sentenciaSql = "SELECT * FROM facturas WHERE idProveedor = '" + idProveedor
                           + "' AND fechaFactura >= '" + fechaInicial + "' AND fechaFactura <= '"
                           + fechaFinal + "'";
        } else {
            idProveedor = base->idProveedor(ui->comboBoxProceedores->currentText(),
                                            conf->getConexionLocal());
            sentenciaSql = "SELECT * FROM albaranes WHERE idProveedor = '" + idProveedor
                           + "' AND fechaFactura >= '" + fechaInicial + "' AND fechaFactura <= '"
                           + fechaFinal + "'";
        }
    }
    ...
    for (int i = 0; i < resultado.numRowsAffected(); ++i) {
        ...
        QStandardItem *itemProveedor = new QStandardItem(
            base->nombreProveedor(resultado.value("idProveedor").toString(),
                                  conf->getConexionLocal()));
        ...
        if (tipoDocumento == "facturas") {
            ...
            QStandardItem *itemFacturada = new QStandardItem(
                resultado.value("facturada").toString());
