...
void comprasVentasRemoto::on_radioButtonVentaMes_clicked()
{
    ...
    modeloVentas.setQuery("SELECT descripcion , YEAR(fecha) , MONTH(fecha) , sum(cantidad) from "
                          "lineasticket WHERE cod = '"
                              + codigo + "' GROUP BY YEAR(fecha) desc , MONTH(fecha) desc",
                          db);
...
void comprasVentasRemoto::on_radioButtonVentasAno_clicked()
{
    ...
    modeloVentas
        .setQuery("SELECT descripcion , YEAR(fecha) , sum(cantidad) from lineasticket WHERE cod = '"
                      + codigo + "' GROUP BY YEAR(fecha) desc",
                  db);
    modeloVentas.setHeaderData(0, Qt::Horizontal, "Artculo");
    modeloVentas.setHeaderData(1, Qt::Horizontal, "Año");
    modeloVentas.setHeaderData(2, Qt::Horizontal, "Cantidad");
    ...
    modeloCompras
        .setQuery("SELECT `nDocumento` , `pedidos`.`idProveedor` , `cantidad` , `bonificacion` , "
                  ...
                  "`pedidos` on `nDocumento` = `pedidos`.`npedido` WHERE `cod` = '"
                      + codigo + "' ORDER BY `pedidos`.`fechaPedido` DESC",
                  db);
...
void comprasVentasRemoto::on_radioButtonComprasMes_clicked()
{
    ...
    modeloCompras.setQuery(
        ...
        "sum(bonificacion) FROM lineaspedido JOIN pedidos ON nDocumento = pedidos.npedido WHERE "
        "cod = '"
            + codigo
    ...
    modeloCompras
        .setQuery("SELECT YEAR(pedidos.fechaPedido) , sum(cantidad) , sum(bonificacion) FROM "
                  "lineaspedido JOIN pedidos ON nDocumento = pedidos.npedido WHERE cod = '"
                      + codigo + "' GROUP BY YEAR(pedidos.fechaPedido) DESC",
                  db);
    ui->tableViewCompras->setModel(&modeloCompras);
    ui->tableViewCompras->resizeColumnsToContents();
}
