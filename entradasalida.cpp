...
void EntradaSalida::on_pushButtonAceptar_clicked()
{
    ...
    datos.append(ui->comboBoxMotivo->currentText().left(ui->comboBoxMotivo->currentText().indexOf("-")));
    qDebug() << ui->comboBoxMotivo->currentText().left(ui->comboBoxMotivo->currentText().indexOf("-"));
    if(base->insertarES(datos)){
...
void EntradaSalida::llenarCombo()
{
    ...
    consulta.first();
    do {
        ui->comboBoxMotivo->addItem(consulta.value("idtiposEntrada").toString()+"-"+consulta.value("descripcion").toString());
    }while(consulta.next());
}
