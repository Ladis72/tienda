...
public:
               ...
               const QModelIndex &index) const override
    {
        ...
        // Dibujamos el texto manualmente para controlar el color dinámico si no está seleccionado
        if (!(opt.state & QStyle::State_Selected)) {
...
void NotasWidget::setupUi()
{
    ...
    tablaView->setToolTip(tr("Doble clic para ver/editar. Clic derecho para más opciones."));
    ...
    titulo->setStyleSheet("font-size: 11pt; font-weight: bold; color: #558b2f; letter-spacing: 1px;");
    
    QPushButton *btnOcultar = new QPushButton("✖", this);
    btnOcultar->setToolTip(tr("Ocultar panel de notas"));
    btnOcultar->setFixedSize(24, 24);
    btnOcultar->setStyleSheet(
        "QPushButton { border: none; background: transparent; color: #9E9E9E; font-weight: bold; }"
    ...
    this->setStyleSheet("QWidget#NotasWidget { background-color: #ffffff; border: 1px solid #e0e0e0; border-radius: 8px; }");
    ...
    connect(cboFiltro, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &NotasWidget::onFiltroChanged);
    connect(tablaView, &QTableView::customContextMenuRequested,
            this, &NotasWidget::onContextMenu);
    connect(tablaView, &QTableView::doubleClicked,
            this, &NotasWidget::onDoubleClicked);
}
...
void NotasWidget::refrescar()
{
    ...
    if (cboFiltro) {
        ...
        if (idx == 1) filtro = "Pendiente";
        else if (idx == 2) filtro = "Completada";
    }
                               ...
                               int idEdicion)
{
    ...
    if (!fechaLimite.isEmpty()) {
        ...
        if (!d.isValid()) d = QDate::fromString(fechaLimite, "yyyy-MM-dd");
        if (d.isValid()) dateLimite->setDate(d);
    }

    QCheckBox *chkSinFecha = new QCheckBox(tr("Sin fecha límite"), &dlg);
    chkSinFecha->setChecked(fechaLimite.isEmpty());
    connect(chkSinFecha, &QCheckBox::toggled, dateLimite, &QDateEdit::setDisabled);
    ...
    fechaLimite = chkSinFecha->isChecked()
                  ? ""
                  : dateLimite->date().toString("yyyy-MM-dd");
    return true;
}
...
void NotasWidget::onNuevaNota()
{
    ...
    if (!base.crearNota(conexionDB, titulo, descripcion, usuario, fechaLimite, prioridad)) {
        QMessageBox::critical(this, tr("Error"), tr("No se ha podido guardar la nota."));
        return;
    }
    refrescar();
}

void NotasWidget::onFiltroChanged(int /*index*/)
{
    refrescar();
}
...
void NotasWidget::onDoubleClicked(const QModelIndex &index)
{
    ...
    if (!base.editarNota(conexionDB, id, titulo, descripcion, fechaLimite, prioridad)) {
        QMessageBox::critical(this, tr("Error"), tr("No se ha podido actualizar la nota."));
        return;
    }
    refrescar();
}
...
void NotasWidget::onContextMenu(const QPoint &pos)
{
    ...
    QString estadoActual = modelo->data(modelo->index(srcIdx.row(), 6)).toString();
    ...
    if (estadoActual != "Completada") {
        ...
        connect(acComp, &QAction::triggered, this, &NotasWidget::onMarcarCompletada);
    } else {
    ...
    for (const QString &p : {tr("Alta"), tr("Normal"), tr("Baja")}) {
        ...
        QString pFinal = p;
        connect(ac, &QAction::triggered, this, [this, pFinal]() {
            onCambiarPrioridad(pFinal);
        });
    }
    menu.addSeparator();

    QAction *acEditar = menu.addAction(tr("✏  Editar nota"));
    connect(acEditar, &QAction::triggered, this, [this, idx]() {
        onDoubleClicked(idx);
    });
