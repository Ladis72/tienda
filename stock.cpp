...
Stock::Stock(QString cod, baseDatos *db, QWidget *parent)
  ...
  if (!registroProd.isEmpty()) {
    ...
    ui->labelProductoHeader->setText(
        QString("Producto: %1 (EAN: %2)")
...
Stock::~Stock() { delete ui; }

void Stock::refrescarLotes() { modeloLotes->select(); }
...
void Stock::on_pushButtonValidar_clicked() {
  ...
  if (filaSeleccionada != -1) {
    // Si la cantidad llega a 0 exactamente, eliminamos el lote.
    // Los lotes con cantidad negativa se mantienen ya que indican errores de stock a corregir.
    if (newCant == 0) {
      q.prepare("DELETE FROM lotes WHERE id = ?");
      q.addBindValue(currentLoteId);
    } else {
      // Actualizar lote existente con los nuevos valores (acepta negativos)
      q.prepare(
          "UPDATE lotes SET cantidad = ?, fecha = ?, lote = ? WHERE id = ?");
    ...
    // Para nuevos lotes, solo los insertamos si la cantidad es distinta de 0
    if (newCant != 0) {
      q.prepare(
          "INSERT INTO lotes (ean, lote, fecha, cantidad) VALUES (?, ?, ?, ?)");
      ...
      // No creamos lotes con cantidad 0
      ok = true;
    }
  }
...
void Stock::on_pushButtonCerrar_clicked() { accept(); }
